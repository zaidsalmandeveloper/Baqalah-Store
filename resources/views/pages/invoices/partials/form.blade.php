...
<div x-data="invoiceForm({
    ...
    <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
        ...
        <div>
            <label for="invoice_number" class="{{ $labelClass }}">Invoice ID</label>
        ...
        <div>
            <x-form.date-picker
                ...
                defaultDate="{{ old('invoice_date', $invoice?->invoice_date?->format('Y-m-d') ?? now()->format('Y-m-d')) }}"
            />
        ...
        <div>
            ...
            <div class="relative z-20 bg-transparent">
                <select name="status" id="status" class="{{ $selectClass }} @error('status') border-error-500 @enderror" required>
                    ...
                    <option value="success" {{ old('status', $invoice?->status) === 'success' ? 'selected' : '' }}>Success</option>
                    <option value="reject" {{ old('status', $invoice?->status) === 'reject' ? 'selected' : '' }}>Reject</option>
                </select>
                <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-500 dark:text-gray-400">
                    <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </span>
        ...
        <div>
            ...
            <input type="number" name="tax_rate" id="tax_rate" step="0.01" min="0" max="100"
                ...
                value="{{ old('tax_rate', $invoice?->tax_rate ?? 15) }}"
    ...
    <div class="mt-8">
        <div class="mb-4 flex items-center justify-between">
            <h4 class="text-base font-semibold text-gray-800 dark:text-white/90">Invoice Items</h4>
